...
from app.schemas.schemes import SchemeMatchRequest, SchemeRequirementsRequest, SchemeResponse
...
@pytest.mark.asyncio
async def test_submit_scheme_requirements_unverified_land_raises_409(scheme_harness):
    """POST /schemes/{scheme_id} (submit requirements) is gated the same as
    /match — never confirms eligibility against unverified land."""
    farm_repo = scheme_harness["farm_repo"]
    service = scheme_harness["service"]
    ...
    with pytest.raises(LandNotVerifiedError) as exc_info:
        await service.submit_scheme_requirements(
            "scheme_tn_01",
            SchemeRequirementsRequest(farm_id=FARM_ID_UNVERIFIED, additional_data={"category": "Small/Marginal"}),
        )

    assert exc_info.value.code == "LAND_NOT_VERIFIED"


@pytest.mark.asyncio
async def test_submit_scheme_requirements_missing_scheme_raises_404(scheme_harness):
    """Verified farm but a scheme id that doesn't exist still 404s."""
    farm_repo = scheme_harness["farm_repo"]
    service = scheme_harness["service"]
    ...
    with pytest.raises(NotFoundError):
        await service.submit_scheme_requirements(
            "scheme_does_not_exist",
            SchemeRequirementsRequest(farm_id=FARM_ID_VERIFIED, additional_data={}),
        )


@pytest.mark.asyncio
async def test_submit_scheme_requirements_verified_farm_returns_scheme_detail(scheme_harness):
    """Verified farm + real scheme id returns the scheme's actual details."""
    farm_repo = scheme_harness["farm_repo"]
    scheme_repo = scheme_harness["scheme_repo"]
    service = scheme_harness["service"]
    ...
    scheme_repo._schemes["scheme_tn_01"] = {
        "id": "scheme_tn_01",
        "name": "Tamil Nadu Paddy Input Subsidy",
        "ministry": "Department of Agriculture, Tamil Nadu",
        "description": "50% subsidy on approved bio-pesticides and micro-nutrients.",
    ...
    result = await service.submit_scheme_requirements(
        "scheme_tn_01",
        SchemeRequirementsRequest(
            farm_id=FARM_ID_VERIFIED,
            additional_data={"category": "Small/Marginal", "annual_income_bracket": "< ₹1.5 Lakhs / Year"},
        ),
    )

    assert isinstance(result, SchemeResponse)
    assert result.id == "scheme_tn_01"
    assert result.name == "Tamil Nadu Paddy Input Subsidy"
