...
Renumbered from its original ``0006`` during the connectivity audit: two
migrations (this one and ``0006_treatment_applications``) had both branched
from ``0005`` with the same revision id ``0006``, which made ``alembic
upgrade head`` fail outright with "Multiple head revisions are present" on
any clean database. They touch unrelated tables, so this one was simply
