...
| `farmer_app` | Voice Query (e2e) | `POST` | `/api/v1/voice/query` | `ApiConstants.voiceQuery` | **BACKEND LIVE, NOT WIRED** | Route called `VoiceService.process_voice_query`, which didn't exist — every call 500'd. Now implemented (transcribe -> `AdvisoryService.answer_query` -> synthesize). The Dart constant still has no call site; `VoiceRepository`/`VoiceController` only drive the transcribe/confirm/synthesize form-filling flow. |
