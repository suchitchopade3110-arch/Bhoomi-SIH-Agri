...
| `farmer_app` | Voice Query (e2e) | `POST` | `/api/v1/voice/query` | `ApiConstants.voiceQuery` | **LIVE** | Route called `VoiceService.process_voice_query`, which didn't exist — every call 500'd. Now implemented (transcribe -> `AdvisoryService.answer_query` -> synthesize). Wired client-side via `VoiceApiService.query()` / `VoiceRepository.askQuestion()`, driven by the dedicated `VoiceQaController` on the `VoiceQaScreen` (`/voice-qa/:farmId`, linked from Ask BHOOMI) — separate from `VoiceController`, which still only drives the transcribe/confirm/synthesize form-filling flow. |
