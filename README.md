...
**The problem-statement switch.** The project was written for SIH25076 (broad farm advisory) and realigned toward SIH26131 (crop disease and pest management). Land registry, revenue-officer review, and irrigation resource planning all stay mounted in both modes now (they're useful trust-building context regardless of problem statement, and `apps/farmer_app`'s Today's Plan screen depends on `resource_plan` being live) — `sih26131` only adds `alerts` and `efficacy` on top of the shared set. `default=sih26131` in `core/config.py` is the live default; the SIH25076 mode exists for the demo-day fallback walkthrough. Note `docs/specs/api_contract_sih26131_delta.md` (§2.1/§2.2/§2.3) is a stale early draft that describes unmounting `land`/`officer`/`schemes`/`resource_plan` under `sih26131` — the team superseded that plan (see `tests/unit/test_problem_statement_gating.py`, the authoritative contract test) without updating the doc.
...
**Ingested — `app/services/rag/corpus_data.py`.** `python -m app.services.rag.ingest` (`make ingest-corpus`) chunks and embeds the hardcoded `CORPUS_DOCS` list there — not the `.md` files in `services/api/corpus/`, which are a human-readable reference mirror, not the ingestion source. 17 disease/agronomy documents (BLB, blast, brown spot, sheath blight, nitrogen management, seed selection, irrigation, harvest timing) plus 8 pest documents (stem borer, brown planthopper, leaf folder, green leafhopper, gall midge, thrips, earhead bug, whorl maggot) — every advisory citation resolves against these 25 docs, and this is the corpus that actually exists at runtime.

**Staged (chemical content only) — `data/curated/Dataset_v4_validated/`.** The same 8 pest documents from TNAU/ICAR-IRRI/IRRI/KVK, sourced with a manifest, structured ETL evidence, and per-chemical regulatory status. Their identification/ETL/cultural/biological-control content was ingested (above); their `chemical_prescriptions` and "Regulatory & Validation Status" sections were deliberately left out — the manifest states `production_ingested: false` and `chemical_advice_status: UNVERIFIED` at the document level, and shipping unvalidated chemical dosages to farmers is a real-world harm, not a demo detail. This split is a demo/prototype call: it lets pest diagnosis honestly compose identification and non-chemical guidance while still escalating on anything needing chemical-specific advice (see §9). Read `DATASET_VALIDATION_STATUS.md` before ever ingesting the chemical portion.
...
~~Pest diagnosis (`target_type=pest`) is wired but never composes advice.~~ **Composes, non-chemical only.** `POST /farms/{id}/diagnose` now retrieves against 5 of the 8 in-scope pest labels — `stem_borer`, `brown_planthopper`, `leaf_folder`, `green_leafhopper`, `gall_midge` — with real identification/ETL/cultural/biological-control content (`kb_p301`–`kb_p308` in `corpus_data.py`; see §8 for what was deliberately left out and why). `fall_armyworm`/`whitefly`/`aphid` have no corpus backing at all and correctly escalate on `NO_RELEVANT_SOURCE`, same as any pest query needing chemical-specific guidance the corpus doesn't carry — both the compose and escalate paths are real and exercised (`tests/test_pest_diagnosis.py`).
